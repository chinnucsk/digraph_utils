...
-export([components/1, strong_components/1, cyclic_strong_components/1, 
	 ...
	 topsort/1, is_acyclic/1, 
         ...
         loop_vertices/1,
	 subgraph/2, subgraph/3, condensation/1, condensation/2,
...
-spec condensation(Digraph, Options) -> CondensedDigraph when
      Digraph :: digraph(),
      Options :: [{'components_as_lables',boolean()} | components_as_labels],
      CondensedDigraph :: digraph().

condensation(G, Opts) ->
    UseLabels = case condensation_opts(Opts, _UseLabels=false) of
                    error ->
                        erlang:error(badarg);
                    UseLabl ->
                        UseLabl
    ...
    SCG = subgraph_opts(G, [], []),
    lists:foreach(fun(SC) -> condense(SC, G, SCG, V2I, I2C, UseLabels)
                  end, SCs),
    ets:delete(V2I),
    ets:delete(I2C),
    SCG.
...
condensation_opts([], UseLabels) ->
    UseLabels;
condensation_opts([{components_as_labels,UseLabels} | Opts], _)
  when is_boolean(UseLabels) ->
    condensation_opts(Opts, UseLabels);
condensation_opts([components_as_labels | Opts], _) ->
    condensation_opts(Opts, true);
condensation_opts(_, _UseLabels) ->
    error.

condense(SC, G, SCG, V2I, I2C, UseLabels) ->
    T = ets:new(condense, []),
    NFun = fun(Neighbour) ->
    ...
    [{_,I1}] = ets:lookup(V2I, hd(SC)),
    case UseLabels of
        true ->
            digraph:add_vertex(SCG, I1, SC);
        false ->
            digraph:add_vertex(SCG, SC)
    end,
    condense(ets:first(T), T, SC, G, SCG, I2C, I1, UseLabels),
    ets:delete(T).

condense('$end_of_table', _T, _SC, _G, _SCG, _I2C, _I1, _UseLabels) ->
    ok;
condense(I, T, SC, G, SCG, I2C, I1, UseLabels) ->
    [{_,C}] = ets:lookup(I2C, I),
    case UseLabels of
        true ->
            digraph:add_vertex(SCG, I, C),
            [digraph:add_edge(SCG, I1, I) || I =/= I1];
        false ->
            digraph:add_vertex(SCG, C),
            [digraph:add_edge(SCG, SC, C) || I =/= I1]
    end,
    condense(ets:next(T, I), T, SC, G, SCG, I2C, I1, UseLabels).
