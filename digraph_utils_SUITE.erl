...
condensation(Config) when is_list(Config) ->
    ...
    do_condense(G, no_opts, not_as_labels),

    %% OTP-9963. The option 'components_as_labels'.
    do_condense(G, [], not_as_labels),
    do_condense(G, [{components_as_labels,false}], not_as_labels),
    do_condense(G, [components_as_labels], as_labels),
    do_condense(G, [{components_as_labels,true}], as_labels),
    ?line {'EXIT',{badarg,_}} =
	(catch digraph_utils:condensation(G, not_a_list)),
    ?line {'EXIT',{badarg,_}} =
	(catch digraph_utils:condensation
                 (G, [{components_as_labels, not_Bool}])),

    true = digraph:delete(G),
    ok.

do_condense(G, OptsSpec, AsLabels) ->
    CG = case OptsSpec of
             no_opts ->
                 digraph_utils:condensation(G);
             Opts ->
                 digraph_utils:condensation(G, Opts)
         end,
    LU = case AsLabels of
             not_as_labels ->
                 fun(V) -> V end;
             as_labels ->
                 fun(V) -> {_V, L} = digraph:vertex(CG, V), L end
         end,
    Vertices = lists:sort(digraph:vertices(CG)),
    [true = lists:seq(1, digraph:no_vertices(CG)) =:= Vertices || AsLabels],
    Vs = sort_2([LU(V) || V <- digraph:vertices(CG)]),
    [[b],[c],[d],[e,f,g],[h],[i,j],[q]] = Vs,
    Fun = fun(E) ->
                  {_E, V1, V2, _L} = digraph:edge(CG, E),
                  {lists:sort(LU(V1)), lists:sort(LU(V2))}
          end,
